        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            while ($produto = $result->fetch_assoc()) {
                ...
                <div class='bg-white p-4 rounded-lg shadow'>
                    <img src='{$produto['imagem']}' alt='{$produto['nome']}' class='w-full h-48 object-cover rounded-t-lg'>
                    <h3 class='text-xl font-semibold mt-4'>{$produto['nome']} - R$ {$produto['preco']}</h3>
                    <p class='mt-2'>{$produto['descricao']}</p>
                    <form method='POST' action='adicionar_carrinho.php'>
                        <input type='hidden' name='produto_id' value='{$produto['id']}'>
                        <button type='submit' class='bg-blue-600 text-white px-4 py-2 mt-4 rounded'>Adicionar ao Carrinho</button>
                    </form>
                </div>";
            }
            ?>
